...
The following table provides an explanation for the various levels of speed.
...
- System scanning activity is logged; not as easily detectable. (Tools such as Wireshark can make such scans detectable)
...
3.**Decoy Scan (-D)**
- The main purpose of this scan is to confuse the target by hiding the real scanning system among multiple decoy systems (Instead of sending traffic from a single machine, Nmap can generate traffic from several specified or randomly generated decoy addresses)
- The real scanning system will still be included in the list of systems that will attempt to scan the target as it is the only system capable of receiving responses from the target
- Can be combined with different kinds of scans
- Detectable by advanced IDS/IPS systems due to traffic patterns
...
- **Special note:** A target's OS can also be roughly guessed by its response to ICMP packets (simply ping the target in command prompt). By checking its TTL value we can come to know if the target is Linux/Unix system (TTL value is around 60) or Windows (TTL value is around 120, double of 60).
...
7.**UDP Scan (-sU)**
- This type of scan is used to scan UDP ports.
- slower than TCP scans as unlike TCP, UDP does not establish a connection hence response traffic is not always guaranteed

8.**Ping Scan (-sn)**
- This type of scan is used to know if a host is alive and responsive.
...
- Useful for scanning targets which block ICMP or host discovery probes through a firewall
...
The following scans attempt to bypass basic firewall filtering by sending packets that do not follow conventional behavior. If these scans successfully bypass firewall filtering and produce results, it is important to carefully analyze the reasoning behind the reported port state.
...
11.** NULL Scan (-sN)**
- In this type of scan, Nmap sends probes with the "NULL" flag set to observe how the target responds. (As a firewall always expects packets to have some sort of flag set in in their header, having no flags can also sometimes bypass the firewall)
  
12.** Xmas Scan (-sX)**
- This kind of scan is known as Xmas Scan due to its use of unexpected trio of TCP flags set (URG, PSH and FIN). As these packets contain 3 distinct flags rather than a single flag in their header, it sometimes is able to evade the firewall. 
